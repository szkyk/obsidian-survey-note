...
## 今後のマイルストーン
- [ ] 下の段の枠は、最低限の高さ以上に文章が行くと表示エリアを広げるように、枠を広げること。現状は枠の中でスクロール担っているが、ページ全体でスクロールされるようにしたい
- [ ] ドラッグアンドドロップで、SurveyNoteプレビューに挿入した画像を、そのままインラインでプレビューできるようにする。
- [ ] ファイル名入力欄以外の入力欄は、フォーカスしたとき、その枠を目立たせるようにする。
- [ ] コードブロックを見やすく表示する
