...
- [x] ```と```で囲んだコードブロックを、コードブロックは、右上にコピーアイコンを設置して、アイコンをクリップしたらコピーできるようにする- [ ] survery viewに切り替えをできるボタンを、ファイルのタイトルバーの近くに置く
- [ ] プロパティを見て、survey-note-view: note なら、デフォルトで、SuveyViewを表示させる
- [ ] -や*による <ul>の実装しして、まるぽちをタップしたときに、それより階層が下のものは閉じたり、開いたり、現在の状態のトグルするようにしたい
- [ ] tableの実装
